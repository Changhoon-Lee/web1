...
#pragma omp critical
 ...
 std::cout<<std::setprecision(30)<<"CRITICAL_RESOLVENT_ITER="<<iter<<" BELOW_ONE="<<below<<" MIN_RATIO="<<global.ratio<<" MIN_INDEX="<<global.index<<" MIN_WEIGHT="<<rd(w.data+4ULL*global.index)<<"\n";}
...
#pragma omp parallel for schedule(static)
 for(uint64_t i=0;i<N;++i){const unsigned kind=i%3;long double e=0;if(kind==0||kind==2){const uint32_t a=kind==0?l1_aux(i):l3_aux(i);const long double mn=std::min<long double>(side[a],std::min<long double>(side[a+AUX],side[a+2*AUX]));e=(kind==0?0.75L:1.5L)*mn;}emission[i]=(float)e;}
 side.clear();side.shrink_to_fit();
 std::vector<float> current(N,0.0f),next(N,0.0f);
 for(unsigned iter=1;iter<=ITER_MAX;++iter){
#pragma omp parallel for schedule(static)
  for(uint64_t i=0;i<N;++i)next[i]=(float)((long double)emission[i]+0.25L*(long double)current[four_index(i)]);
  current.swap(next);
