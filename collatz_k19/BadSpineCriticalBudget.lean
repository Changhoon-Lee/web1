...
factor strictly larger than `33/32`.  This file records the exact rational fact
that the K19 aggregate retention is larger than `32/33`, so the two certified
...
actual hypothetical bad component realizes retention at least `32/33`.
-/
...
/-- The exact all-row K19 aggregate retention clears the reciprocal side budget. -/
theorem k19AggregateRetention_gt_thirtyTwo_over_thirtyThree :
    (32 : ℝ) / 33 < k19AggregateRetention := by
  norm_num [k19AggregateRetention]

/-- Exact scalar closure: the certified `33/32` side factor pays the complete
all-row K19 aggregate deficit with strict margin. -/
theorem side_factor_times_k19AggregateRetention_gt_one :
    (1 : ℝ) < (33 : ℝ) / 32 * k19AggregateRetention := by
  norm_num [k19AggregateRetention]
...
theorem composition_of_side_and_occupation
    {side retention : ℝ}
    (hside : (33 : ℝ) / 32 < side)
    (hretention : (32 : ℝ) / 33 ≤ retention) :
    1 < side * retention := by
  have hpositive : (0 : ℝ) < (32 : ℝ) / 33 := by norm_num
  have hscaled := mul_lt_mul_of_pos_right hside hpositive
  have hlower : side * ((32 : ℝ) / 33) ≤ side * retention := by
...
#print axioms BadSpineCriticalBudget.k19AggregateRetention_gt_thirtyTwo_over_thirtyThree
