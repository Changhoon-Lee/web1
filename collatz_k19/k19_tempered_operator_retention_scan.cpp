...
static constexpr unsigned ITER_MAX=24;
...
static Minimum scan_ratio(const std::vector<float>&cur,const std::vector<float>&base,long double threshold,uint64_t&below){Minimum g;below=0;
#pragma omp parallel
 {Minimum l;uint64_t b=0;
#pragma omp for schedule(static)
  for(uint64_t i=0;i<N;++i){long double r=(long double)cur[i]/base[i];if(r<threshold)++b;if(r<l.ratio)l={r,i};}
#pragma omp critical
  {below+=b;if(l.ratio<g.ratio)g=l;}}
 return g;}
...
int main(int argc,char**argv){if(argc!=2)return 2;Mapping w(argv[1]);if(w.bytes!=4ULL*N)return 3;
 ...
 const std::array<unsigned,5> Qs={16,32,64,128,256};
 std::cout<<std::setprecision(30)<<"ROWS="<<N<<"\n";
 for(unsigned Q:Qs){std::vector<float> base(N),cur(N),next(N);
#pragma omp parallel for schedule(static)
  for(uint64_t i=0;i<N;++i){float v=(float)std::pow((long double)rd(w.data+4ULL*i),1.0L/Q);base[i]=v;cur[i]=v;}
  std::cout<<"Q="<<Q<<"\n";
...
#pragma omp parallel for schedule(static)
   ...
   cur.swap(next);
   if(iter<=4||iter==6||iter==8||iter==12||iter==16||iter==20||iter==24){uint64_t belowFiveNinths=0,belowOne=0;Minimum m=scan_ratio(cur,base,5.0L/9.0L,belowFiveNinths);Minimum m1=scan_ratio(cur,base,1.0L,belowOne);std::cout<<"ITER="<<iter<<" MIN_RATIO="<<m.ratio<<" MIN_INDEX="<<m.index<<" BELOW_5_OVER_9="<<belowFiveNinths<<" BELOW_ONE="<<belowOne<<" MIN_CHECK_ONE="<<m1.ratio<<"\n";}
  }
 }
 return 0;}
