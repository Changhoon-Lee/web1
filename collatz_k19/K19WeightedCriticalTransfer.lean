...
* `W` be a common coefficient normalizer,
* `M` be the weighted harmonic mass of a finite equal-depth antichain, and
...
and the exact disjoint-reservoir capacity is at most `H`, then necessarily

`delta * D ≤ 1`.

Consequently, constructing a lawful target antichain with `delta * D > 1`
gives an immediate contradiction at the exact critical exponent. This is the
corrected positive live arrow after the harmonic-ceiling no-go.
-/
...
/-- Exact equal-depth predecessor capacity forces the weighted critical product
to be at most one. -/
theorem weighted_critical_product_le_one
...
critical density satisfies `delta * D > 1` contradicts the exact single-cutoff
capacity bound. -/
theorem contradiction_of_weighted_critical_overflow
    ...
    (hlower : (delta / W) * H ^ gamma * M ≤ total)
    (hcapacity : total ≤ H)
    (hoverflow : 1 < delta * D) : False := by
  have hceiling := weighted_critical_product_le_one hdelta hW hH
