...
#pragma omp parallel for schedule(static)
  for(uint64_t i=0;i<N;++i){u128 numerator=u128(C0)*value[four_index(i)];unsigned k=i%3;if(k==0||k==2){uint32_t a=k==0?l1_aux(i):l3_aux(i);uint32_t mn=std::min<uint32_t>(value[a],std::min<uint32_t>(value[a+AUX],value[a+2*AUX]));numerator+=u128(k==0?C1:C3)*mn;}u128 quotient=numerator/COEFFICIENT_SCALE;if(quotient>std::numeric_limits<uint32_t>::max()){overflow.store(true,std::memory_order_relaxed);fresh[i]=std::numeric_limits<uint32_t>::max();}else fresh[i]=uint32_t(quotient);}
  if(overflow.load(std::memory_order_relaxed)){std::cout<<"UINT32_OVERFLOW=FAIL\nEXACT_CERTIFICATE=OPEN\n";return 5;}
  uint64_t below=0,belowOne=0,changed=0;uint32_t minValue=std::numeric_limits<uint32_t>::max(),maxValue=0;uint64_t minIndex=0;
#pragma omp parallel
  {uint64_t b=0,b1=0,ch=0;uint32_t lmin=std::numeric_limits<uint32_t>::max(),lmax=0;uint64_t lidx=0;
#pragma omp for schedule(static)
   for(uint64_t i=0;i<N;++i){uint32_t x=fresh[i];if(uint64_t(8)*x<=uint64_t(7)*VALUE_SCALE)++b;if(x<VALUE_SCALE)++b1;if(x>value[i])++ch;if(x<lmin){lmin=x;lidx=i;}if(x>lmax)lmax=x;value[i]=std::max(value[i],x);}
#pragma omp critical
   {below+=b;belowOne+=b1;changed+=ch;if(lmin<minValue){minValue=lmin;minIndex=lidx;}if(lmax>maxValue)maxValue=lmax;}}
  if(iter<=16||iter%16==0||below==0||changed==0){std::cout<<"ITER="<<iter<<" MIN_VALUE="<<minValue<<" MIN_INDEX="<<minIndex<<" MIN_RATIO_NUM="<<minValue<<" MIN_RATIO_DEN="<<VALUE_SCALE<<" BELOW_OR_EQUAL_7_OVER_8="<<below<<" BELOW_ONE="<<belowOne<<" VALUE_CHANGED="<<changed<<" MAX_VALUE="<<maxValue<<"\n";}
  if(below==0){std::cout<<"EXACT_UNWEIGHTED_BELLMAN_FIRST_PASS="<<iter<<"\nUINT32_OVERFLOW=PASS\nEXACT_CERTIFICATE=PASS\n";return 0;}
